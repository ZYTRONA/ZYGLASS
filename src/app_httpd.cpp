...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    // Target 60 FPS - delay 16ms between frames
    ...
    if (res != ESP_OK) {
      // Client disconnected or network error - exit gracefully and allow reconnect
      break;
    }
    int64_t fr_end = esp_timer_get_time();
    ...
    // Frame rate limiting: ensure minimum 16ms (60 FPS) between frames
    int64_t elapsed = (fr_end - frame_start) / 1000; // Convert to ms
    if (elapsed < 16) {
      vTaskDelay((16 - elapsed) / portTICK_PERIOD_MS);
    }
  }
...
static esp_err_t index_handler(httpd_req_t *req) {
  ...
  const char* html = "<html><body><h1>ZYGLASS Camera</h1><img src='/stream'></body></html>";
